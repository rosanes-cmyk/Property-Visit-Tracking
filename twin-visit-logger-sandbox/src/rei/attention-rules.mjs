/**
 * Which of the 8 work-queue buckets a lead is in — the SAME rules the Chat card uses.
 *
 * The re-check needs this to answer one question: "is this lead on the card right now?" Leads that are get
 * looked at hourly; the rest rotate through in the background. The client: "we need to prioritise those 8
 * buckets in updating and checking… that is the main goal."
 *
 * The rules below are a VERBATIM copy of apps-script/ChatNotify.gs, not a translation of it. Hand-translating
 * them into Node would have created a second definition that drifts — which is exactly what produced
 * "Upcoming Visit: 0" and "no visit date set" on a preview that was supposed to be identical to the card.
 * tests/rei-buckets asserts this copy and the .gs are the same text, so they cannot disagree.
 *
 * Regenerate after editing ChatNotify.gs:  node scripts/sync-attention-rules.mjs
 */

/* The two helpers the rules call, matching Code.combined.gs exactly. */
const fmt_ = (d) => {
  const x = new Date(d);
...
/* Script Properties, matching apps-script/Config.gs. */
const CFG = { DIGEST_INCLUDE_IMPORTED: false };
...
/**
 * The bucket key a lead is in, or '' when it is not on the card at all.
 *
 * `today` is injectable so a test is not at the mercy of the clock.
 */
export function bucketOf(row, today = today_()) {
  const hit = attentionBucket_(row || {}, today);
  const gift = giftPending_(row || {});
  if (hit) return hit.key;
  return gift ? 'giftFollowUp' : '';
}

/** Is this lead on the 3pm card right now? */
export function onTheCard(row, today = today_()) {
  return bucketOf(row, today) !== '';
}

export { attentionBucket_, giftPending_, excludedFromDigest_, ATTENTION_BUCKETS };
